...
defmodule PlateSlate.Menu do
  ...
  def list_categories(args) do
    list_schema(Category, args)
  end

  defp list_schema(model, args) do
    args
  ...
  def list_items(args) do
    list_schema(Item, args)
  end

  defp filter_with(query, filter, Item) do
  ...
  defp filter_with(query, filter, Category) do
    Enum.reduce(filter, query, fn
      {:name, name}, query ->
        from(q in query, where: ilike(q.name, ^"%#{name}%"))
    end)
